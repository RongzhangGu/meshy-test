...
import {clamp, catalogueLayout, openingLayout, heroComposition, heroPlacements, heroCallouts} from './opening-layout.js';
import {OPENING_SETTLE, openingProgress, createCatalogueStop, catalogueDockPosition, holdCatalogueScroll} from './opening-scroll.js';
...
export default function CreativeOpening({query,catalogueFilter,onFilter,onCreate,startExpanded,tools}) {
  ...
  useEffect(()=>{
    ...
    let frame=0, width=0, height=0, top=0, travel=0, settledTravel=0, inset=0;
    let stop=null, stopUsed=false, lastWheel=0, previousY=window.scrollY, unlockTimer=0;
    ...
    function releaseStop(){
      clearTimeout(unlockTimer);
      stop=null;stopUsed=true;lastWheel=0;
    ...
    function draw(time,settled=false){
      ...
      openingLayout(width,height,progress,0,mobile,shownProducts.length,openingProducts.length).forEach((pose,index)=>{
        ...
        const placement=heroPlacements[products.findIndex(product=>product.id===shownProducts[index].id)];
        card.style.setProperty('--hero-caption-x',`${(placement?placement[6]-placement[0]:0)*labelUnit}px`);
        card.style.setProperty('--hero-caption-y',`${(placement?placement[7]-placement[1]-placement[3]:0)*labelUnit}px`);
    ...
    function scroll(){
      ...
      if(!stop&&y<target-settledTravel*.5)stopUsed=false;
    ...
    function wheel(event){
      if(!canStop()||event.defaultPrevented||event.ctrlKey||Math.abs(event.deltaX)>=Math.abs(event.deltaY))return;
      ...
      if(delta<0){if(stop)resumeScroll(event,delta);else releaseStop();return;}
    ...
    return()=>{releaseStop();cancelAnimationFrame(frame);resize.disconnect();element.removeEventListener('lab:browse',browseFromNavigation);window.removeEventListener('scroll',scroll);window.removeEventListener('wheel',wheel);window.removeEventListener('keydown',releaseStop);window.removeEventListener('pointerdown',pointerDown);reduced.removeEventListener('change',measure);};
  ...
  return <section className="kinetic-catalog" ref={root} aria-label="Creative Lab creations">
    ...
    <div className="opening-sticky" ref={scene}>
      <header className="opening-toolbar">
        <h1 className="lab-section-title" id="page-title" tabIndex={-1}>Browse Creations<span className="seed-rest" data-seed-anchor="catalog" aria-hidden="true"/></h1>
        <div className="opening-controls">
          <nav className="opening-filters" aria-label="Creation filters">
            {catalogueFilters.map(filter=><button key={filter.id} aria-pressed={catalogueFilter===filter.id} onClick={()=>onFilter(filter.id)}>{filter.label}</button>)}
          </nav>
          {tools}
        </div>
      </header>
      <div className="ribbon-scene">
        <div className="opening-wordmark" aria-hidden="true"><span>Creative</span><span>Lab<span className="opening-period" data-seed-anchor="hero"/></span></div>
        <p className="opening-note opening-note--ideas" aria-hidden="true">Turn your ideas<br/>into 3D creations</p>
        <p className="opening-note opening-note--customize" aria-hidden="true">Customize.<br/>Print. Share.</p>
        {shownProducts.map((product,index)=><button key={product.id} ref={element=>cards.current[index]=element} className="opening-model" style={{'--float-duration':`${6.8+index%5*.55}s`,'--float-delay':`${-index*.67}s`}} data-creation={product.id} aria-label={`${product.mode==='device'?'Explore':'Try Now:'} ${product.title}`} aria-describedby={`creation-description-${product.id}${product.badge ? ` creation-badge-${product.id}` : ''}`} onClick={()=>onCreate(product.id)} onFocus={event=>{if(event.currentTarget.matches(':focus-visible'))reveal.current();}}>
          ...
          <span className="opening-model-caption">
            <span className="opening-model-title">
              {heroCallouts[product.id]&&<svg className="opening-sketch-arrow" viewBox="0 0 1 1" fill="none" aria-hidden="true" focusable="false">
                <defs><marker id={`hero-callout-${product.id}`} markerWidth="9" markerHeight="9" refX="7" refY="4" orient="auto" markerUnits="userSpaceOnUse"><path d="M1 1L7 4 1 7"/></marker></defs>
                <path d={heroCallouts[product.id]} markerEnd={`url(#hero-callout-${product.id})`}/>
              </svg>}
