...
import {openingProgress, OPENING_SETTLE, createCatalogueStop, catalogueDockPosition, holdCatalogueScroll} from './opening-scroll.js';
...
test('opening reaches the catalogue continuously without a final layout snap', () => {
  ...
  assert.ok(openingProgress(end - 1, 260) > .99);
...
test('a fast wheel lands smoothly at the catalogue without overshooting', () => {
  ...
  const positions = [1000, 1060, 1120, 1180, 1240, 1300, 1360, 2000].map(now => catalogueDockPosition(stop, now));
...
test('arrival absorbs momentum and preserves a short fully expanded rest', () => {
  ...
  for (const now of [0, 100, 359, 360, 500, 779]) assert.equal(holdCatalogueScroll(stop, 2000, now), true);
});
