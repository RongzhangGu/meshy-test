...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (disabled || reduced.matches || document.hidden || !visible || (!focused && (style.visibility === 'hidden' || Number(style.opacity) === 0))) {
        setActive(false); return;
      }
      const rect = btn.getBoundingClientRect(), pointer = pointerRef.current;
      const distance = pointer ? Math.hypot(Math.max(rect.left - pointer.clientX, 0, pointer.clientX - rect.right), Math.max(rect.top - pointer.clientY, 0, pointer.clientY - rect.bottom)) : Infinity;
      setActive(autoAnimate || focused || (followMouse && distance < proximity));
    };
    const move = event => { pointerRef.current = {clientX:event.clientX,clientY:event.clientY}; update(); };
  ...
  useEffect(() => {
    ...
    catch { return; } // The CSS material remains usable without WebGL.
    ...
    let proximityT = 0;
    let surfaceTarget = 35, surfaceX = 35;
    ...
    const update = now => {
      ...
      const brightTarget = p.autoAnimate || (btn.closest('button') || btn).matches(':focus-visible') ? 1 : proximityT;
