...
import { advanceSeedJourney, businessMarkMotion, pointOnSeedPath, seedFlightPosition, advanceSeeds } from './brand-journey.js';
...
test('business underline finishes after landing and retracts before departure on the reversible trip clock', () => {
  ...
  assert.deepEqual(arrival(.6),{ink:0,flight:1,drawing:true});
  assert.deepEqual(arrival(1),{ink:1,flight:1,drawing:true});
  assert.deepEqual(departure(0),{ink:1,flight:0,drawing:true});
  assert.deepEqual(departure(.4),{ink:0,flight:0,drawing:true});
  assert.deepEqual(departure(1),{ink:0,flight:1,drawing:false});
  for(let i=0;i<=100;i++) {
    const p=i/100;
    assert.ok(Math.abs(arrival(p).ink-departure(1-p).ink)<1e-12,'reverse travel retraces the same ink');
  }
  ...
  assert.ok(Math.abs(businessMarkMotion(state).ink-.5)<1e-12,'ink advances with time after the wheel stops');
  state=advanceSeedJourney(state,{target:1,now:2100,duration:1400});
  assert.ok(businessMarkMotion(state).ink<1e-12,'a reversal erases before returning to the preceding section');
  assert.equal(businessMarkMotion(advanceSeedJourney(state,{target:2,now:2200,reducedMotion:true})).ink,1);
});
...
test('section trips finish by time, rest between destinations, and reverse the same progress', () => {
  ...
  assert.equal(state.progress,.4,'the trip advances without another wheel event');
  state=advanceSeedJourney(state,{target:1,now:1300});
  assert.equal(state.progress,.2,'reversing retraces the running trip');
...
test('section flights use the interior, recede and reverse through identical positions', () => {
  ...
  for(let i=0;i<=1000;i++){
    ...
    forward.push(p);
    assert.ok(p.x>=90 && p.x<360,'the route stays inside the composition rather than the left gutter');
    assert.ok(p.y>=route.a.y && p.y<=route.b.y);
    const previous=forward.at(-2);
    if(previous) assert.ok(Math.hypot(p.x-previous.x,p.y-previous.y)<1,'continuous spatial arc');
  }
...
test('the mark lands exactly at either anchor and particles stay finite after a pause or collision', () => {
  const a={x:280,y:120}, b={x:220,y:1500};
  const path=[a,{x:280,y:154},{x:16,y:154},{x:16,y:1470},{x:220,y:1470},b];
  ...
  for(let i=1;i<=1000;i++) {
    ...
    assert.ok(Math.hypot(p.x-previous.x,p.y-previous.y)<2.1,'no discontinuity as the source leaves the viewport');
    if(p.y>154 && p.y<1470)assert.equal(p.x,16,'travel stays outside the content');
    previous=p;
  }
  const balls=[{x:0,y:0,vx:0,vy:0,homeX:-1,homeY:1,r:.2},{x:0,y:0,vx:0,vy:0,homeX:1,homeY:-1,r:.2}];
  for(let i=0;i<500;i++) advanceSeeds(balls,i===0?8:1/60,{x:0,y:0},{x:2.65,y:2.65});
  for(const ball of balls) for(const key of ['x','y','vx','vy']) assert.ok(Number.isFinite(ball[key]));
  for(const ball of balls) {assert.ok(Math.abs(ball.x)<2.66);assert.ok(Math.abs(ball.y)<2.66);}
  assert.ok(Math.hypot(balls[0].x-balls[1].x,balls[0].y-balls[1].y)>=.39);
});
...
test('My creations arrival falls onto the M, clears the title and reverses without jumps', () => {
  ...
  for(let i=0;i<=1000;i++){
    const p=seedFlightPosition({...route,progress:i/1000});
    forward.push(p);
    if(p.x>route.landing.x && p.x<route.b.x && i>650) assert.ok(p.y<=route.landing.y,'the hop stays above the text');
    const previous=forward.at(-2);
    if(previous) assert.ok(Math.hypot(p.x-previous.x,p.y-previous.y)<10,'no jumps at the landing or hop');
  }
  const contact=seedFlightPosition({...route,progress:.7375});
  assert.ok(Math.hypot(contact.x-route.landing.x,contact.y-route.landing.y)<1e-9);
  assert.deepEqual(forward.at(-1),{...route.b,recess:0});
  for(let i=1000;i>=0;i--) assert.deepEqual(seedFlightPosition({...route,progress:i/1000}),forward[i]);
});
