...
test('every photo creation has its own verified Create controls and available examples',()=>{
  assert.deepEqual(Object.keys(creationSettings).sort(),products.filter(p=>!p.inputKind).map(p=>p.id).sort());
  ...
  const expectedExamples={keychain:4,chibi:5,vinyl:5,brick:4,magnet:0,keycap:0,pixel:4,pet:4,lamp:9,egg:9,collapsible:9,plantpot:9};
  for(const [id,settings] of Object.entries(creationSettings)){
    assert.equal(settings.examples.length,expectedExamples[id],id);
    for(const example of settings.examples)assert.ok(existsSync(new URL(`../public/assets/${example.image}.webp`,import.meta.url)),example.image);
  }
  assert.equal(validateImage({type:'image/png',size:15*1024*1024},creationSettings.lamp.maxMB),'');
  assert.match(validateImage({type:'image/png',size:15*1024*1024},creationSettings.chibi.maxMB),/10 MB/);
});
