...
export default function Discovery({
  ...
  return (
    <section
      className={`discovery-scene ${selected ? 'is-editing' : 'has-opening'}`}
    ...
    >
        ...
        <>
          <header className="workspace-header">
            <nav className="workspace-breadcrumb" aria-label="Breadcrumb">
              <ol>
                <li>
                  <button onClick={onCollapse}>Creative Lab</button>
                </li>
                <li>
                  <CaretRight size={14} aria-hidden="true" />
                  <h1 id="page-title" aria-current="page">
                    {product.title}
                  </h1>
                </li>
              </ol>
            ...
            {product.mode !== 'device' && (
              <ol className="workspace-progress" aria-label="Creation progress">
                {steps.map((label, index) => (
                  ...
                  >
                    <span className="workspace-step-number" aria-hidden="true">
      ...
      <div id="workbench-panel" className="catalog-workbench" hidden={!selected}>
        {children}
      </div>
    </section>
  );
