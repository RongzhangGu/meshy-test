  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        const edgeOpacity = 1 - smoothstep(fadeStart, 1, edge);
        ...
        // Fade the whole object and caption before either becomes an isolated clipped fragment.
        const visibleTravel = Math.max(1, (height - cardHeight * visualScale) / 2);
        const viewportOpacity = 1 - smoothstep(Math.max(0, visibleTravel - 24), visibleTravel + 48, Math.abs(offset * verticalSpacing * fit));
        const opacity = edgeOpacity * viewportOpacity;
      ...
      <div className="infinite-spiral__stage" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          ...
          return (
            ...
            >
              {item.spritePosition ? <span className="infinite-spiral__image product-atlas" role="img" aria-label={item.alt} style={{backgroundImage:`url(${item.src})`,backgroundPosition:item.spritePosition}}/> : <img
              ...
              />}
              {item.label && <span className="infinite-spiral__caption">{item.label}</span>}
            </Card>
          );
        })}
      </div>
