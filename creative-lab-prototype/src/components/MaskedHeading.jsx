...
export default function MaskedHeading({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const progress = Math.max(
        -1,
        Math.min(1, (innerHeight / 2 - rect.top - rect.height / 2) / (innerHeight / 2)),
      );
    ...
    const measure = () => {
      ...
      if (line) {
        ...
        line
          ...
          .forEach((char) => char.style.setProperty('--char-left', `${char.offsetLeft}px`));
      }
      schedule();
    };
  ...
  return (
    <Tag
      ...
      data-reveal={reveal}
      style={{
        '--masked-image': `url("${src}")`,
        '--masked-fill': `${fillScale * 100}%`,
        '--masked-scale': fillScale,
      }}
    >
      {reveal === 'float' ? (
        <ScrollFloat
          as="span"
          textClassName="masked-heading-float"
        ...
        <span className="masked-heading-fill">{text}</span>
      )}
    </Tag>
  );
}
