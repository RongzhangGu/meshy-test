...
export default function TiltedCard({
  ...
  useEffect(() => {
    ...
    media.add(
      ...
      () => {
        ...
        const toX = gsap.quickTo(surface.current, 'rotationX', {
          duration: 0.65,
          ease: 'power3.out',
        });
        const toY = gsap.quickTo(surface.current, 'rotationY', {
          duration: 0.65,
          ease: 'power3.out',
        });
        const toScaleX = gsap.quickTo(surface.current, 'scaleX', {
          duration: 0.65,
          ease: 'power3.out',
        });
        const toScaleY = gsap.quickTo(surface.current, 'scaleY', {
          duration: 0.65,
          ease: 'power3.out',
        });
  ...
  return (
    <figure className="tilted-card" ref={card}>
      <div className="tilted-card-surface" ref={surface}>
        <div className="tilted-card-image">
          <img
            src={imageSrc}
            alt={altText}
            loading="lazy"
      ...
      {captionText && (
        <figcaption className="tilted-card-caption" ref={caption} aria-hidden="true">
          {captionText}
        </figcaption>
      )}
    </figure>
