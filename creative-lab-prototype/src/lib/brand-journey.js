...
export function advanceSeedJourney(state, { target, now, reducedMotion = false, duration = 1000 }) {
  ...
  if (state.from === state.to) {
    ...
    return {
      ...
      progress: target > state.from ? 0 : 1,
      lastTime: now,
    };
  }
  const direction = target <= state.from ? -1 : 1;
  const progress = clamp01(
    state.progress + (direction * Math.max(0, now - state.lastTime)) / duration,
  );
...
export function seedFlightPosition({ a, b, progress, bend = 80, landing }) {
  ...
  if (landing && t >= 0.65) {
    const u = (t - 0.65) / 0.35;
    if (u < 0.25) return { x: landing.x, y: landing.y - 64 + 64 * (u / 0.25) ** 2, recess: 0 };
    if (u < 0.8) {
      const hop = (u - 0.25) / 0.55;
      return {
        x: landing.x + (b.x - landing.x) * hop,
        y: landing.y - 88 * hop * (1 - hop),
        recess: 0,
      };
    }
    return { x: b.x, y: landing.y + (b.y - landing.y) * smooth01((u - 0.8) / 0.2), recess: 0 };
  }
  const end = landing ? { x: landing.x, y: landing.y - 64 } : b;
  const u = landing ? t / 0.65 : t,
    v = 1 - u,
    dy = end.y - a.y;
  return {
    x:
