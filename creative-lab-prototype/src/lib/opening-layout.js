...
export function heroComposition(width, height) {
  ...
  return {
    scale,
    left: (width - 2048 * scale) / 2,
    top: Math.max(0, (height - 92 - 888 * scale) / 2),
  };
}
...
export function catalogueLayout(width, mobile = width < 700, count = 14) {
  ...
  return {
    ...
    cardScale,
    orbitHeight,
    height: top + rows * cardHeight + (rows - 1) * rowGap + 24,
  };
}
...
export function openingLayout(
  ...
  const { columns, gap, rowGap, top, cardWidth, cardHeight, cardScale, orbitHeight } =
    catalogueLayout(width, mobile, count);
  const composition = heroComposition(width, height);
  // Reserve breathing room for the tools and filters even on shorter desktops.
  const orbitScale = mobile ? 1 : Math.min(1, (height - 120) / Math.min(width * 0.54, 860));
  const radius = (mobile ? width * 0.38 : Math.min(width * 0.38, 760)) * orbitScale;
  const tilt = mobile ? 0.1 : (20 * Math.PI) / 180;
  return Array.from({ length: count }, (_, index) => {
    const row = Math.floor(index / columns);
    const inRow = Math.min(columns, count - row * columns);
    const finalX =
      (width - inRow * cardWidth - (inRow - 1) * gap) / 2 + (index % columns) * (cardWidth + gap);
    ...
    if (index >= openingCount || (!mobile && !heroPlacements[index])) {
      const appear = clamp((p - 0.45) / 0.55);
      const reveal = appear * appear * (3 - 2 * appear);
      return {
        x: finalX,
        y: finalY + 32 * (1 - reveal),
        z: 0,
        rotateY: 0,
        rotateZ: 0,
    ...
    if (!mobile) {
      ...
      const centerY =
        composition.top + (y + h / 2) * composition.scale + 22 * cardScale * projection;
      ...
      return {
        ...
        scale: 1,
        opacity: 1,
        width: heroWidth + (cardWidth - heroWidth) * ease,
    ...
    const orbitY =
      height * (mobile ? 0.48 : 0.45) +
      along * Math.sin(tilt) +
      across * Math.cos(tilt) -
      orbitHeight / 2;
    const baseScale = ((mobile ? 165 : Math.min(345, width * 0.228)) * orbitScale) / cardWidth;
    // A compact silhouette and a deeper orbit leave space for the complete collection.
    const scale = baseScale * (0.83 + depth * 0.16);
    return {
      x: orbitX + (finalX - orbitX) * ease,
      y: orbitY + (finalY - orbitY) * ease,
      z: depth * (mobile ? 95 : 235) * (1 - ease),
      rotateY: -Math.sin(angle) * 32 * (1 - ease),
      rotateZ: (((tilt * 180) / Math.PI) * 0.4 - Math.sin(angle) * 7) * (1 - ease),
      scale: scale + (1 - scale) * ease,
      opacity: (0.7 + (depth + 1) * 0.15) * (1 - ease) + ease,
      width: cardWidth,
      height: orbitHeight + (cardHeight - orbitHeight) * ease,
    };
  });
}
