...
import { getWorkflowScrollState, getWorkflowWheelStop, getWorkflowReadingScrollY, isWorkflowTitleReady, holdWorkflowStep, initialWorkflow, workflowTransition } from './workflow-transition.js';
...
export default function Workflow({ onStartCreation, precedingRef }) {
  ...
  useLayoutEffect(() => {
    ...
    window.scrollTo({ top: getWorkflowReadingScrollY(scrollY, top, nextTop, delta), behavior: 'instant' });
  ...
  useEffect(() => {
    ...
    const finishJourney = (delta = 0) => {
      wheelStop = null;
      readingScroll.current = { top:section.querySelector('.workflow-demo').getBoundingClientRect().top, delta };
      setReading(true);
    };
    const draw = () => {
      frame = 0;
      ...
      const targetSpread = expanded.current ? 1 : titleReady ? smooth((openingProgress - .24) / .5) : 0;
      ...
      lastDraw = now;
      spread = reduced || compact ? 1 : !titleReady ? 0 : spread + (targetSpread - spread) * (1 - Math.exp(-elapsed / 180));
      ...
      if (!reduced && !compact && spread > .98 && !expanded.current) {
    ...
    const schedule = () => {
      ...
      if (wheelStop && Math.abs(scrollY - wheelStop.y) > .5) window.scrollTo({ top: wheelStop.y, behavior: 'instant' });
      if (!frame) frame = requestAnimationFrame(draw);
    };
    redraw.current = schedule;
    const releaseWheel = () => { wheelStop = null; lastWheel = 0; };
    const wheel = event => {
      if (reduced || compact || event.defaultPrevented || event.ctrlKey || Math.abs(event.deltaX) >= Math.abs(event.deltaY)) return;
      if (event.target.closest('input,textarea,select,[role="dialog"],.nav-search-panel')) return;
      const delta = event.deltaY * (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? innerHeight : 1);
      if (delta < 0) { releaseWheel(); return; }
      const now = performance.now();
      const quietFor = now - lastWheel;
      lastWheel = now;
      if (wheelStop) {
        const turning = section.querySelector('.workflow-artwork').dataset.phase === 'turning';
        const hold = wheelStop.phase === 'title'
      ...
      const stop = getWorkflowWheelStop(offset, delta, distance, innerHeight, isWorkflowTitleReady(titleReveal.current, now));
      if (!stop) return;
      if (event.cancelable) event.preventDefault();
      wheelStop = { y: scrollY + stop.offset - offset, since: now, phase: stop.phase, step: stop.step };
      window.scrollTo({ top: wheelStop.y, behavior: 'instant' });
      if (stop.phase !== 'title') dispatch({ type: 'select', step: stop.step });
      schedule();
    };
    const resize = () => { releaseWheel(); schedule(); };
    const pointerDown = event => {
      if (event.pointerType === 'touch' || event.target.closest('a,button,input,textarea,select') || event.clientX >= document.documentElement.clientWidth) releaseWheel();
    };
  ...
  return <section className="lab-workflow" id="how-it-works" aria-labelledby="workflow-title" ref={demo} data-motion={reduced || compact ? 'static' : reading ? 'read' : 'scroll'} style={reading ? {'--workflow-entry':1,'--workflow-spread':1,'--workflow-copy':1,'--workflow-reveal':1} : undefined}>
    {steps.map(item => <link key={item.image} rel="preload" as="image" href={`/assets/workflow-${item.image}.webp`}/>)}
    <div className="workflow-stage" ref={stage}>
      <div className="workflow-sheet">
        <span className="workflow-guide" aria-hidden="true"/>
        <h2 className="workflow-title lab-section-title" id="workflow-title" aria-label="MESHY CREATIVE LABS: How & Why It Works">
          <MaskedHeading as="span" className="workflow-title-context" text="MESHY CREATIVE LABS" src="/assets/workflow-choose.webp" fillScale={1.3} parallax={12} reveal="float" triggerRef={demo} staticMotion={reduced || compact || introSeen} once stagger={titleStagger} onRevealChange={complete => updateTitleReveal('context', complete)}/>
          <MaskedHeading as="span" className="workflow-title-main" text="How & Why It Works" src="/assets/workflow-choose.webp" fillScale={1.3} parallax={34} reveal="float" triggerRef={demo} staticMotion={reduced || compact || introSeen} once stagger={titleStagger} onRevealChange={complete => updateTitleReveal('main', complete)}/>
        </h2>
        ...
        <div className="workflow-intro" aria-hidden={revealed}>
          <span>Scroll to discover <ArrowDown size={15} aria-hidden="true"/></span>
        </div>
        <div className="workflow-demo">
          <div className="workflow-visual">
            <div key={state.step} id="workflow-card" className="workflow-artwork" role="img" aria-label={steps[activeStep].alt} data-step={state.step + 1} data-next-step={state.next + 1} data-phase={state.phase} style={{'--workflow-turn':`${-180 * state.direction}deg`}}
              onAnimationEnd={event => { if (event.target === event.currentTarget) dispatch({ type: 'turned' }); }}>
              <div className="workflow-face" aria-hidden="true"><img className="workflow-illustration" src={`/assets/workflow-${step.image}.webp`} alt="" width="1920" height="1080"/></div>
              <div className="workflow-face workflow-face-back" aria-hidden="true"><img className="workflow-illustration" src={`/assets/workflow-${incomingStep.image}.webp`} alt="" width="1920" height="1080"/></div>
            </div>
          </div>
          <div className="workflow-navigation" inert={!revealed}>
            <ol className="workflow-steps" aria-label="How & Why It Works steps">
              {steps.map((item, index) => <li key={item.title} className={activeStep === index ? 'is-current' : index < activeStep ? 'is-past' : undefined}>
                <button className="workflow-step" aria-current={activeStep === index ? 'step' : undefined} aria-expanded={activeStep === index} aria-controls={`workflow-description-${index}`} onClick={() => selectStep(index)}>
                  <span className="workflow-step-number"><i className="workflow-seed-anchor" data-seed-anchor={activeStep === index ? "workflow" : undefined} aria-hidden="true"/>Step {index + 1}</span><span>{item.title}</span>
                </button>
                <div className="workflow-step-copy" id={`workflow-description-${index}`} aria-hidden={activeStep !== index} inert={activeStep !== index}><div className="workflow-step-copy-inner">
