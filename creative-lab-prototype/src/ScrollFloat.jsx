...
export default function ScrollFloat({ children, scrollContainerRef, triggerRef, containerClassName = '', textClassName = '', animationDuration = 1, ease = 'back.inOut(2)', scrollStart = 'center bottom+=50%', scrollEnd = 'bottom bottom-=40%', stagger = .03, scrub = true, as: Tag = 'h2', disabled = false, once = false, onRevealChange }) {
  const containerRef = useRef(null);
  const revealChange = useRef(onRevealChange);
  revealChange.current = onRevealChange;
  ...
  useEffect(() => {
    ...
    if (disabled) { clearMotion(); revealChange.current?.(true); return; }
    const media = gsap.matchMedia();
    media.add('(prefers-reduced-motion: no-preference)', () => {
      revealChange.current?.(false);
      gsap.set(characters, {
      ...
      const animation = gsap.to(characters, {
        ...
        onStart: () => revealChange.current?.(false),
        onComplete: () => revealChange.current?.(true),
        scrollTrigger: {
          trigger: triggerRef?.current || element,
          scroller: scrollContainerRef?.current || window,
          start: scrollStart, end: scrollEnd, scrub, once, toggleActions: once ? 'play none none none' : 'play none none reset',
          onLeaveBack: () => { if (!once) revealChange.current?.(false); }
        }
      });
  ...
  }, [text, scrollContainerRef, triggerRef, animationDuration, ease, scrollStart, scrollEnd, stagger, scrub, disabled, once]);
