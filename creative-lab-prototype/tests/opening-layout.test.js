...
import { catalogueLayout, openingLayout } from '../src/lib/opening-layout.js';
...
test('the orbit resolves into fourteen equally sized, non-overlapping, reachable categories', () => {
  for (const [width, height] of [
    ...
    if (width >= 1200) {
      ...
      assert.deepEqual(
        [...new Set(tiles.map((tile) => tile.y))].map(
          (y) => tiles.filter((tile) => tile.y === y).length,
        ),
        [6, 6, 2],
      );
      assert.ok(
        Math.abs(layout.cardHeight - 136 * layout.cardScale - layout.cardWidth) < 0.01,
        'the creation gets a full square image area above its caption',
      );
    }
    for (const [i, tile] of tiles.entries()) {
      assert.equal(tile.z, 0);
      assert.equal(Math.abs(tile.rotateY), 0);
      assert.equal(Math.abs(tile.rotateZ), 0);
      assert.equal(tile.scale, 1);
      assert.ok(tile.x >= -0.01 && tile.x + tile.width <= width + 0.01);
      assert.ok(tile.y >= 0 && tile.y + tile.height <= catalogueLayout(width).height + 0.01);
      for (const other of tiles.slice(i + 1))
        assert.ok(
          tile.x + tile.width <= other.x + 0.01 ||
            other.x + other.width <= tile.x + 0.01 ||
            tile.y + tile.height <= other.y + 0.01 ||
            other.y + other.height <= tile.y + 0.01,
        );
    }
...
test('the reference composition reveals the two omitted products when the full catalogue unfolds', () => {
  ...
  const partial = openingLayout(1376, 772, 0.7);
