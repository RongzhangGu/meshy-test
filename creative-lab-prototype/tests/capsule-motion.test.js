...
import {
  ...
  resolveSeedShellContacts,
  CAPSULE_IMPACT,
  CAPSULE_SECOND_IMPACT,
  CAPSULE_TRAVEL_MS,
  CAPSULE_TIMING,
...
test('particles bounce off moving, tilted hemispheres and their rims throughout opening and closing', () => {
  ...
  const side = { x: 1.15, y: 0.3, z: 0, r: 0.2, vx: -2, vy: 0 };
  const rim = { x: 1.14, y: -0.05, z: 0, r: 0.2, vx: 0, vy: 2 };
  resolveSeedShellContacts([side, rim], [shell]);
  assert.ok(side.vx > 0, 'inward velocity reflects from the outer surface');
  assert.ok(rim.y < -0.2 && rim.vy < 0, 'the open rim also has a collision boundary');
  const carried = { x: 0, y: 1.3, z: 0, r: 0.15, vx: 0, vy: 0 };
  resolveSeedShellContacts([carried], [{ ...shell, y: 0.3, vy: 2 }]);
  ...
  const balls = Array.from({ length: 8 }, (_, i) => ({
    ...
    y: Math.sin(i) * 1.9,
    z: ((i % 3) - 1) * 0.35,
    homeX: Math.cos(i) * 1.9,
    homeY: Math.sin(i) * 1.9,
    r: 0.12 + (i % 3) * 0.08,
    vx: 0,
    vy: 0,
  }));
  for (let frame = 0; frame < 720; frame++) {
    const phase = frame / 60,
      open = (1 - Math.cos(phase * 2)) * 0.5,
      angle = Math.sin(phase) * 0.35;
    const shells = [
      {
        ...shell,
        x: -open * 0.24,
      ...
      { ...shell, x: open * 0.12, y: -open * 0.42, nx: Math.sin(angle), ny: -Math.cos(angle) },
    ];
    for (let step = 0; step < 4; step++) {
      advanceSeeds(
        balls,
        1 / 120,
        { x: Math.cos(phase * 3) * 1.6, y: Math.sin(phase * 3) * 1.6 },
        { x: 2.65, y: 2.65 },
      );
    ...
    for (const b of balls)
      for (const s of shells) {
        const dx = b.x - s.x,
          ...
          dz = b.z - s.z;
        assert.ok(
          Math.hypot(dx, dy, dz) >= s.radius + b.r + 0.011 ||
            dx * s.nx + dy * s.ny + dz * s.nz <= -b.r - 0.011,
...
test('capsule springs open continuously, overshoot gently and settle independently of frame rate', () => {
  ...
  for (const fps of [30, 60, 120]) {
    ...
    for (let frame = 0; frame < fps * 3; frame++) {
      ...
      assert.ok(Math.abs(spring.position - previous) < 0.18, 'no instant open assignment');
    ...
    assert.ok(Math.abs(spring.position - 1) < 0.00001);
    assert.ok(Math.abs(spring.velocity) < 0.00001);
  }
...
test('impact compresses the shell before dissipating and reversing a spring preserves its position', () => {
  let impact = { position: 0, velocity: 3 },
    peak = 0;
  for (let frame = 0; frame < 180; frame++) {
    impact = advanceCapsuleSpring(impact, 0, 1 / 60, true);
    peak = Math.max(peak, impact.position);
  }
  assert.ok(peak > 0.08 && peak < 0.15);
  assert.ok(Math.abs(impact.position) < 0.00001);
  ...
  assert.ok(
    Math.abs(closing.position - opening.position) < 0.1,
    'reversing changes force, not the current pose',
  );
});

test('footer flight lands at its dock, rebounds above it and retraces the same continuous path', () => {
  const route = { a: { x: 400, y: 900 }, b: { x: 1050, y: 1750 } },
    samples = [];
  ...
  assert.deepEqual(capsuleFlightPosition({ ...route, progress: CAPSULE_IMPACT }), {
    ...route.b,
    recess: 0,
  });
  assert.deepEqual(samples.at(-1), { ...route.b, recess: 0 });
  assert.equal(
    capsuleFlightPosition({ ...route, progress: (CAPSULE_IMPACT + CAPSULE_SECOND_IMPACT) / 2 }).y,
    route.b.y - 44,
  );
  for (let i = 1000; i >= 0; i--)
    assert.deepEqual(capsuleFlightPosition({ ...route, progress: i / 1000 }), samples[i]);
  let trip = advanceSeedJourney(null, { target: 5, now: 0 });
  trip = advanceSeedJourney(trip, { target: 6, now: 700, duration: CAPSULE_TRAVEL_MS });
  trip = advanceSeedJourney(trip, {
    target: 6,
    now: 700 + CAPSULE_TRAVEL_MS / 2,
    duration: CAPSULE_TRAVEL_MS,
  });
  assert.equal(trip.progress, 0.5);
  trip = advanceSeedJourney(trip, {
    target: 6,
    now: 700 + CAPSULE_TRAVEL_MS,
    duration: CAPSULE_TRAVEL_MS,
  });
...
test('the dot docks before its matched-size handoff and grows without an endpoint snap', () => {
  assert.deepEqual(capsuleArrival(0), {
    flight: 0,
    handoff: 0,
    growth: 0,
    letterImpact: 0,
  ...
  assert.deepEqual(capsuleArrival(1), {
    flight: 1,
    handoff: 1,
    growth: 1,
    letterImpact: 0,
  ...
  for (let i = 0; i <= 1000; i++) {
    ...
    if (p.handoff > 0)
      assert.equal(p.flight, 1, 'renderers exchange only after the landing is complete');
    if (p.growth > 0)
      assert.equal(p.handoff, 1, 'growth cannot start while the DOM dot is visible');
    assert.ok(p.growth >= 0 && p.growth < 1.12);
    const previous = samples.at(-1);
    if (previous) assert.ok(Math.abs(p.growth - previous.growth) < 0.02);
    samples.push(p);
  }
  assert.ok(1 - capsuleArrival(0.9999).growth < 0.00001, 'growth settles without an endpoint snap');
...
test('the first hit squashes and releases the letter; only the second hit absorbs it', () => {
  ...
  const touch = CAPSULE_TIMING.firstHit / CAPSULE_TRAVEL_MS,
    secondTouch = CAPSULE_TIMING.secondHit / CAPSULE_TRAVEL_MS;
  const before = capsuleArrival(touch - 0.04),
    at = capsuleArrival(touch),
    pressed = capsuleArrival(touch + 50 / CAPSULE_TRAVEL_MS);
  assert.equal(before.letterImpact, 0, 'anticipation must not look like an early collision');
  assert.deepEqual(capsuleFlightPosition({ ...route, progress: at.flight }), {
    ...route.contact,
    recess: 0,
  });
  assert.equal(at.letterAbsorb, 0, 'the full letter is still present at contact');
  assert.equal(pressed.letterImpact, 0.85);
  assert.equal(pressed.letterAbsorb, 0, 'show the compression before dissolving the glyph');
  assert.equal(
    capsuleArrival(secondTouch - 0.02).letterImpact,
    0,
    'the letter recovers before the second hit',
  );
  for (let p = touch; p <= secondTouch; p += 0.005)
    assert.equal(
      capsuleArrival(p).letterAbsorb,
      0,
      'the full letter remains throughout the rebound',
    );
  const second = capsuleArrival(secondTouch);
  assert.deepEqual(capsuleFlightPosition({ ...route, progress: second.flight }), {
    ...route.contact,
    recess: 0,
  });
  ...
  assert.equal(
    capsuleArrival(CAPSULE_TIMING.open / CAPSULE_TRAVEL_MS).letterAbsorb,
    1,
    'the letter is absorbed as the local sphere takes over',
  );
  assert.deepEqual(capsuleFlightPosition({ ...route, progress: 1 }), { ...route.b, recess: 0 });
  let previous = capsuleArrival(0),
    point = capsuleFlightPosition({ ...route, progress: 0 });
  for (let i = 1; i <= 1000; i++) {
    const next = capsuleArrival(i / 1000),
      nextPoint = capsuleFlightPosition({ ...route, progress: next.flight });
    for (const key of ['letterImpact', 'letterAbsorb']) {
      assert.ok(next[key] >= 0 && next[key] <= 1);
      assert.ok(Math.abs(next[key] - previous[key]) < 0.08, 'the letter has no pose discontinuity');
    }
    assert.ok(
      Math.hypot(nextPoint.x - point.x, nextPoint.y - point.y) < 7,
      'contact and dock remain one continuous path',
    );
    previous = next;
    point = nextPoint;
  }
  ...
  for (const fps of [30, 60, 120]) {
    ...
    for (let frame = 1; frame <= fps * 2.5; frame++) {
      const elapsed = (frame / fps) * 1000,
        arrival = capsuleArrival(elapsed / CAPSULE_TRAVEL_MS);
      spring = advanceCapsuleSpring(spring, arrival.handoff === 1 ? 1 : 0, 1 / fps);
      if (arrival.growth > 0 && arrival.growth < 1 && spring.position > 0.1) overlap = true;
      if (Math.abs(spring.position - 1) >= 0.015 || Math.abs(spring.velocity) >= 0.12)
        lastUnsettled = elapsed;
    }
