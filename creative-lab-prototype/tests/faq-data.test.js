...
import { faqGroups, filterFaqGroups } from '../src/data/faq-data.js';
...
test('FAQ search finds answer-only keywords regardless of case and whitespace', () => {
  ...
  assert.deepEqual(
    results[0].items.map(([question]) => question),
    ['What file formats do I get?', 'Will it print well on my 3D printer?'],
  );
  assert.equal(
    filterFaqGroups('AI handles')[0].items[0][0],
